...
class SupertrendBot:
    def __init__(
        ...
        # prefer largest timeframe first
        ...
        # Load all USDT markets, exclude stablecoin bases, sort by 24h volume, then take top 500
        if symbols:
            raw = symbols
        else:
        ...
        self.symbols = sorted(
            ...
            reverse=True
        )[:500]  # only the top 500 by volume
    ...
    def _update_reports(self):
        ...
        # === TRADES.CSV with fees and reordered columns ===
        if not df.empty:
            df = df.copy()
            ...
            col_order = [
                ...
                'symbol',
                # Entry details
                'entry_tf', 'entry_time', 'entry_price', 'size', 'size_usdt',
                # Exit details
                'exit_time', 'exit_price', 'exit_usdt',
                # Performance
                'pnl', 'cumulative_pnl', 'cumulative_pnl_pct',
                # Fees
                'total_fees_usd', 'cumulative_fees_usd',
                # Misc
                'duration_min'
            ]
        ...
        if not df.empty:
            ...
            monthly.to_csv(
                os.path.join(RESULT_DIR, 'monthly_summary.csv'),
                index=False
            )

        # === TIMEFRAME-WISE SUMMARY ===
        if not df.empty:
            tf_df = df.copy()
            ...
            tf_summary.to_csv(
                os.path.join(RESULT_DIR, 'timeframe_summary.csv'),
                index=False
            )
    ...
    def send_discord_report(self):
        ...
        # Realised PnL aggregation
        if not df.empty and 'pnl' in df.columns:
            realised_pnl = df['pnl'].sum()
        ...
        # Calculate unrealised PnL on open positions
        unrealised_pnl = 0.0
        for pos in self.open_positions.values():
            curr_price = self.exchange.fetch_ticker(pos['symbol'])['last']
            # fees could be included if desired: ((pos['entry_price'] * pos['size'] + curr_price * pos['size']) * self.fee_pct)
        ...
        # Append each open position detail
    ...
    def _trade_cycle(self):
        ...
        # DEBUG: single-pair flips check
        sample = self.symbols[0]
        for tf in self.timeframes:
            o = self.exchange.fetch_ohlcv(sample, tf, limit=50)
            df = pd.DataFrame(o, columns=['ts','open','high','low','close','vol'])
            df = self.calculate_supertrend(df)
            flips = [
                i for i in range(1, len(df))
                if not df['in_uptrend'].iat[i-1] and df['in_uptrend'].iat[i]
            ]
            print(f"[DEBUG] {sample}@{tf} flips at indices: {flips}")
            ...
            with ThreadPoolExecutor(max_workers=8) as ex:
                ...
                for fut in as_completed(futures):
                    ...
                    if r:
                        if not signals:
                            sym, tf, price, _ = r
                            print(f"--- DEBUG: first flip → {sym}@{tf} @ {price:.4f} ---")
                        signals.append(r)
                        if len(signals) >= needed:
                            break

            if signals:
                # prioritize by timeframe: 1d > 4h > 1h
    ...
    def run_live(self):
        try:
            # Use allocated_balance as initial balance if provided
